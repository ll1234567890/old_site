...
<%
	...
	try {
		...
		rs.open("select yy.F1,yy.F2,yid.F2 as F6,yid2.F2 as F7,yy.F8,yy.F9 from y.txt yy,yid.txt yid,yid2.txt yid2 where yid.F1=yy.F6 and yid2.F1=yy.F7 order by yy.F1 desc,yy.F2 desc","DSN=trans");
		...
		rs2.open("select count(F1) from y.txt","DSN=trans");
